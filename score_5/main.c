...
int main(int argc, char *argv[]) {
    ...
    pid_t pid_input, pid_main, pid_output;
    int p_status;
    ...
    if (pid_input == 0) {
        ...
        if (pid_main == 0) {
            ...
            // создаю процесс для вывода ответа в файл
            pid_output = fork();
            if (pid_output < 0) {
    ...
    if (waitpid(pid_input, &p_status, 0) < 0 || waitpid(pid_main, &p_status, 0) < 0 ||
        waitpid(pid_output, &p_status, 0) < 0) {
